...
Intersail::ZfClient.configure do |config|
  ...
  config.base_uri = "http://srvdelivery.staging.vpn"

  ###
  # Optional settings
  ###

  # In case you want to use a different base_uri for each manager you can set a
  # custom one for each manager and do not set the config.base_uri
...
=== opzione raw
Nel caso si passi al client il metodo "raw" il risultato ritornato sarà sempre un'hash, e.g.:
    client.user_manager.raw.read(1)

== Models
Il metodo attributes di un model contiene un hash in cui le chiavi rappresentano gli attributi da serializzare,
ed i valori rappresentano i valori di default degli attributi in caso siano valorizzati con nil.